...
- [x] 2.1 `knowledge-compiler` — **DECIDIDO 2026-06-10**: nuevo `internal/compiler/` package, único writer a `knowledge_objects` (ADR-003), 7 preguntas en orden (existencia Q1-Q3, graph build Q4-Q6, human review Q7), transacción `SERIALIZABLE` con 4-6 escrituras atómicas, proceso separado `cmd/compiler`. `agreement_score` queda como stub (ADR-002) hasta el follow-up de #12. 2-3 semanas. Ver `research/02-1-knowledge-compiler.md`.
...
- [x] 2.3 `agents-shared-brain` — **DECIDIDO 2026-06-10**: `AgentRuntime` como única superficie pública de escritura. 6 agents como stubs (`internal/agent/`), Research concreto. `IngestTextService` deprecada. HTTP + Telegram handlers van por el runtime. La exploración `research-agent` se archiva. 2 semanas. Ver `research/02-3-agents-shared-brain.md`.
...
- [x] 3. Brechas de `exploration.md` — todas mapeadas a un change (ver tabla §3)
- [x] 4. Quick wins ejecutados (#11, #13, #15) — los 3 analizados y commiteados en `2c40a8b`
